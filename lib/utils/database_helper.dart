...
class DatabaseHelper {
  ...
  Future<Map<String, dynamic>> updateUser({
    ...
    try {
      ...
      if (phone != null) updateData['phone'] = phone.replaceAll(RegExp(r'[^\d]'), '');
      ...
      return {
        ...
        'message': updatedRows > 0 ? 'Cập nhật thành công!' : 'Không tìm thấy người dùng!',
      };
    } catch (e) {
  ...
  Future<Map<String, dynamic>> changePassword({
    ...
    try {
      ...
      final user = users.first;
      if (!_verifyPassword(oldPassword, user['password'] as String)) {
  ...
  Future<Map<String, dynamic>> resetPassword({
    ...
    try {
      ...
      final db = await database;
      final users = await db.query('users', where: 'email = ?', whereArgs: [email.toLowerCase()]);
  ...
  Future<Map<String, dynamic>> deleteUser(int userId) async {
    try {
      final db = await database;
      int deletedRows = await db.delete('users', where: 'id = ?', whereArgs: [userId]);

      return {
        'success': deletedRows > 0,
        'message': deletedRows > 0 ? 'Xóa tài khoản thành công!' : 'Không tìm thấy tài khoản!',
      };
    } catch (e) {
